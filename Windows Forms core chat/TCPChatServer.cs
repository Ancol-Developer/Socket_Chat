    ...
    public class TCPChatServer : TCPChatBase
    {
        ...
        public void ReceiveCallback(IAsyncResult AR)
        {
           ...
           AddToChat( text );
            ...
            else if (text.ToLower().Contains(common.USERNAME))
            {
                string username = text.Replace(common.USERNAME + " ", "");
                if (username.Length == 0)
                {
                    AddToChat($"username is been not empty");
                }
                else
                {
                    // create new username
                    if(currentClientSocket.userName == null)
                    {
                        if (CheckValidUserName(username))
                        {
                            ...
                            byte[] data = Encoding.ASCII.GetBytes("Assign successfull ");
                            currentClientSocket.socket.Send(data);
                            AddToChat($"{username} is joined to the chat");
                        }
                        else
                        {
                            // Always Shutdown before closing
                    ...
                    {
                        // thieu validate trong truong hop update ten
                        string oldname = currentClientSocket.userName;
                        currentClientSocket.userName = username;
                        AddToChat($"{oldname} is update to {username}");
                        byte[] data = Encoding.ASCII.GetBytes("Update username successfull ");
                        currentClientSocket.socket.Send(data);
                    }
                }
            }
        ...
        public bool CheckValidUserName(string userName_Check)
        {
            foreach (ClientSocket c in clientSockets)
            {
                if (c.userName == userName_Check)
                {
                    return false;
                }
            }
